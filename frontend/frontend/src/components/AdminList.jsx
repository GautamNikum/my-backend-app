        ...
        <tbody>
          {admins.map((admin) => (
            <tr key={admin._id}>
              ...
              <td>{admin.role}</td>
              <td>
                <button onClick={() => onEdit(admin)}  style={{backgroundColor:"royalblue", marginRight:"10px", marginLeft:"10px"}}>  Edit</button>
                <button onClick={() => deleteAdmin(admin._id)} style={{backgroundColor:"crimson"}}>Delete</button>
              </td>
            </tr>
          ))}
          {admins.length === 0 && (
